...
export async function handleRecommendationRoute(
  ...
  if (request.method === "GET") {
    const snapshot = await db.collection("recommendations")
      ...
      .limit(20)
      .get();
    response.json({recommendations: snapshot.docs.map((document: any) => ({recommendationId: document.id, ...document.data()}))});
    return;
  }
...
function buildRecommendation(userId: string, profile: Record<string, unknown> | undefined): Record<string, unknown> {
